...
| **ADR-010** | **Giám Sát & Chẩn Đoán Từ Xa ESP Insights, Kênh MQTT Dùng Chung & Bắt Lỗi Core Dump** | [ADR-010.md](../ADR-010-esp-insights-remote-diagnostics.md) | [**Xem Sơ Đồ HTML**](./ADR-010-esp-insights-remote-diagnostics.html)<br>([JSON](./ADR-010-esp-insights-remote-diagnostics.architecture.json)) | 1. *Shared MQTT & Memory Optimization* (Dùng chung kênh MQTT RainMaker, tiết kiệm 30-40 KB SRAM)<br>2. *Quantitative Metrics & Custom Events* (Thu thập Heap, Wi-Fi RSSI & sự kiện đèn `ESP_DIAG_EVENT`)<br>3. *Crash Simulation & Core Dump Recovery* (Bấm 5 lần kích hoạt crash thử nghiệm, lưu ELF dump vào phân vùng 64KB Flash, tự động gửi lên cloud khi reboot)<br>4. *Dual-Target Hardware Platform* (Xác thực trên cả ESP32-C3 và ESP32-S3 với layout 1920KB dual OTA) |
